...
insert  into `comment`(`commentId`,`content`,`createTime`,`commentLikeCount`,`commentCommentCount`,`userId`,`postId`,`rootCommentId`,`toCommentId`) values (1,'test用户1（于postId=1）评论1：这有一个新的游戏','2000-01-01',0,0,3,1,-1,-1),(2,'test用户1（于postId=3）评论1：看上去还不错','2000-01-01',0,0,3,3,-1,-1),(3,'test用户2（于postId=3）评论1：有点意思','2000-01-01',0,0,4,3,-1,-1),(4,'test用户2（于postId=5）评论1：我来帮你，我做个攻略','2000-01-01',0,0,4,5,-1,-1),(5,'test用户1（于postId=5回复CommentId=4）评论1：谢谢大佬','2000-01-01',0,0,3,5,4,4),(6,'test用户2（于postId=5回复commentId=5）评论2：攻略已经发出来啦','2000-01-01',0,0,4,5,4,5),(7,'test用户1（于postId=6）评论1：感谢大佬的帮助！！！','2000-01-01',0,0,3,6,-1,-1),(8,'test用户2（于postId=6回复commentId=7）评论1：小菜一碟~~~','2000-01-01',0,0,4,4,7,7);
...
CREATE TABLE `post` (
  ...
  `cover` varchar(100) NOT NULL DEFAULT '$(postId)Cover.png' COMMENT '帖子封面',
  ...
  `tagId` int(15) NOT NULL DEFAULT '1' COMMENT '帖子tag',
  `userId` int(15) NOT NULL COMMENT '作者的用户id',
  PRIMARY KEY (`postId`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=gbk;

/*Data for the table `post` */

insert  into `post`(`postId`,`title`,`content`,`createTime`,`cover`,`postLikeCount`,`postBookmarkCount`,`postCommentCount`,`tagId`,`userId`) values (1,'test管理员1标题1：新的游戏论坛','test管理员1内容1','2000-01-01','1Cover.png',0,0,0,1,1),(2,'test管理员1标题2：更新公告','test管理员1内容2','2000-01-01','2Cover.png',0,0,0,1,1),(3,'test管理员2标题1：新的活动！','test管理员2内容1','2000-01-01','3Cover.png',0,0,0,1,2),(4,'test用户1标题1：好玩，爱玩','test用户1内容1','2000-01-01','4Cover.png',0,0,0,4,3),(5,'test用户1标题2：打不过求助','test用户1内容2','2000-01-01','5Cover.png',0,0,0,3,3),(6,'test用户2标题2：***攻略','test用户2内容2','2000-01-01','6Cover.png',0,0,0,2,4);
...
insert  into `tag`(`tagId`,`name`) values (1,'更新公告'),(2,'游戏攻略'),(3,'玩家求助'),(4,'分享日常');
...
CREATE TABLE `user` (
  ...
  `bookmarkPost` varchar(5000) DEFAULT 'null' COMMENT '收藏的帖子',
  `likeComment` varchar(5000) DEFAULT 'null' COMMENT '点赞的评论',
  `verifyCode` char(4) DEFAULT 'null' COMMENT '验证码',
  `token` varchar(255) DEFAULT 'null' COMMENT '用户请求头',
  PRIMARY KEY (`userId`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=gbk;

/*Data for the table `user` */

insert  into `user`(`userId`,`type`,`account`,`password`,`nickname`,`birthday`,`email`,`avatar`,`followUser`,`likePost`,`bookmarkPost`,`likeComment`,`verifyCode`,`token`) values (0,0,'111','123456789','test管理员1','2000-01-01','test1@itcast.cn','0Avatar.png','null','null','null','null',NULL,'null'),(1,0,'222','123456789','test管理员2','2000-01-01','test2@itcast.cn','1Avatar.png','null','null','null','null',NULL,'null'),(2,1,'111111','123456789','test用户1','2000-01-01','test用户1@itcast.cn','2Avatar.png','null','null','null','null',NULL,'null'),(3,1,'222222','123456789','test用户2','2000-01-01','test用户2@itcast.cn','3Avatar.png','null','null','null','null',NULL,'null');
